...
Доказать $g_{,k} = g g^{ij}g_{ij,k}$.

Предлагаю рассмотреть равенство:

\begin{equation}
    \ln\inner{\det\inner{g_{ij}}} = \Tr \ln{g_{ij}} = \delta^{j}_{i} \ln{g_{ij}}
\end{equation}

Продифференциируеме его, спарава получим:

\begin{equation}
    \piv{\delta^{j}_{i} \ln{g_{ij}}}{x^k} = 
    \delta^{j}_{i} \piv{\ln{g_{ij}}}{x^k} = \delta^{j}_{i} \cfrac{g_{nj,k}}{g_{ni}} = 
    \delta^{j}_{i} g^{ni}g_{nj,k} = g^{ni}g_{ni,k} 
\end{equation}

Слева получим:

\begin{equation}
    \piv{\ln\inner{g}}{x^k} = \cfrac{g_{,k}}{g}
\end{equation}

Объединим:

\begin{equation}
    \cfrac{g_{k}}{g} = g^{ni}g_{ni,k} \implies g_{,k} = g g^{ni}g_{ni,k}
\end{equation}
